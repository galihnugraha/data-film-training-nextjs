...
const getUpComingMovie = async () => {
  return satellite.get("https://api.themoviedb.org/3/movie/upcoming", {
    headers: {
    ...
    .then(res => {
      console.log("RESPONSE UP COMING MOVIE :", res.data)
      return res.data
    })
    .catch(err => console.log("RESPONSE UP COMING MOVIE ERROR :", err))
}

export default async function UpCpming() {
  ...
  return (
    <div className="min-h-screen grow p-6">
      <div className="h-[110px] w-full rounded-lg bg-white px-5 py-6 justify-center items-center">
          <h3 className='text-center text-3xl'>Up Coming Movies</h3>
          <p className='text-center'>{upComing.dates.minimum} until {upComing.dates.maximum}</p>
      </div>

      <div className="mt-7 h-fit w-full rounded-lg bg-white p-7">
        <div className="grid grid-cols-3 gap-10">
          {upComing?.results.map((movie: APIThemoviedb) => (
